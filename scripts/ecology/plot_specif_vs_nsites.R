...
load("analyses/ecology/nostoc_module_assignments.RData")
...
peltigera_summary <- abmi_id_data %>%
  ...
  mutate(color = ifelse(module %in% c(2, 3, 4), "#892b46", "gray50"))
...
peltigera_plot <- ggplot(peltigera_summary, aes(x = n_sites, y = n_partners)) +
  ...
  scale_y_continuous(breaks = seq(0, 8, by = 2)) +  # Set y scale breaks
