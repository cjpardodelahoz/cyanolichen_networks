...
#SBATCH --output=log/lichen_sequencing/ont_lichen/polish_its_%A_%a.out
#SBATCH --error=log/lichen_sequencing/ont_lichen/polish_its_%A_%a.err
...
reads_path="analyses/lichen_sequencing/ont_lichen/demultiplex/its"
out_path="analyses/lichen_sequencing/ont_lichen/error_correction/its"

# Sample variable
sample=$(cat analyses/lichen_sequencing/ont_lichen/demultiplex/all_its_barcodes_plus.tsv | cut -f 1 | sed -n ${SLURM_ARRAY_TASK_ID}p)
#sample=$(cat analyses/lichen_sequencing/ont_lichen/demultiplex/all_its_barcodes_plus.tsv | cut -f 1 | sed -n 1p)
...
# Run a blastn search against the Peltigera ITS sequences from Pardo-de la Hoz et al. 2024 Then, 
...
for i in `seq 1 2`; do
    medaka_consensus -i ${out_path}/${sample%%_plus}_filtered.fastq \
        ...
        -o ${out_path}/medaka_its_${sample}_${i} \
        -t 1
    rm ${out_path}/${sample%%_plus}_polished.fasta.*
    mv ${out_path}/medaka_its_${sample}_${i}/consensus.fasta ${out_path}/${sample%%_plus}_polished.fasta
done
# Remove Medaka temporary files
rm -r ${out_path}/medaka_its_${sample}_*

# Replace header with sample name
sed -i "s/>.*/>${sample%%_its_plus}/" ${out_path}/${sample%%_plus}_polished.fasta

# Print read count used for polishing
n_reads=$(grep -c "^+$" ${out_path}/${sample%%_plus}_filtered.fastq)
echo "${sample%%_its_plus},${n_reads}" > ${out_path}/${sample%%_plus}_read_count.csv